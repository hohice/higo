...
var ErrInvalidLogLevel = herrors.NewDefineError("logger: invalid log level")
...
// HigoLogger logs logging records to the specified io.Writer
type HigoLogger struct {
	mu     sync.Mutex
	output io.Writer
}
...
var Log = GetHigoLogger(os.Stdout)
...
// GetHigoLogger initializes the logger instance with a NewColorWriter output
// and returns a singleton
func GetHigoLogger(w io.Writer) *HigoLogger {
	once.Do(func() {
		var (
			err             error
		...
		instance = &HigoLogger{output: colors.NewColorWriter(w)}
	})
	return instance
}

// SetOutput sets the logger output destination
func (l *HigoLogger) SetOutput(w io.Writer) {
	l.mu.Lock()
	defer l.mu.Unlock()
...
func (l *HigoLogger) getLevelTag(level int) string {
	switch level {
	case levelFatal:
		return "FATAL   "
	case levelSuccess:
	...
	case levelCritical:
		return "CRITICAL"
	default:
		herrors.Panic(ErrInvalidLogLevel)
	}
	return ""
}

func (l *HigoLogger) getColorLevel(level int) string {
	switch level {
	case levelCritical:
...
func (l *HigoLogger) mustLog(level int, message string, args ...interface{}) {
	if level > logLevel {
		return
	}
	// Acquire the lock
...
func (l *HigoLogger) mustLogDebug(message string, file string, line int, args ...interface{}) {
	if !debugMode {
		return
	}
